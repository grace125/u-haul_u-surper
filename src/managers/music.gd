...
func play_song(new_song: String, volume: float = DEFAULT_MUSIC_VOLUME, fade_duration: float = DEFAULT_CROSSFADE_TIME) -> void:
	if new_song != current_song:
		...
		music_player.play()
		var tween2 = get_tree().create_tween().set_ease(MUSIC_CROSSFADE_TWEEN_EASE).set_trans(MUSIC_CROSSFADE_TWEEN_TRANS)
		tween2.tween_property(music_player, "volume_db", volume, fade_duration)
